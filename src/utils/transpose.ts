...
export function transposeScale(notes: string[], root: string): string[] {
const semitones = NOTES.length;
const targetRootIndex = NOTES.indexOf(root);
if (targetRootIndex === -1) throw new Error(`Invalid root note: ${root}`);
...
if (newIndex < 0) newIndex += semitones;
return NOTES[newIndex];
});
}

export function transposeChords(chords: string[], root: string): string[] {
const semitones = NOTES.length;
const targetRootIndex = NOTES.indexOf(root);
if (targetRootIndex === -1) throw new Error(`Invalid root note: ${root}`);
